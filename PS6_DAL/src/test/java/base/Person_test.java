...
public class Person_Test {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		Date dDate = null;
		try {
			dDate = new SimpleDateFormat("yyyy-MM-dd").parse("1996-12-29");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		
		per1 = new PersonDomainModel();
		per1.setFirstName("Robbie");
		per1.setLastName("Yacucci");
		per1.setBirthday(dDate);
		per1.setCity("Wilmington");
		per1.setPostalCode(19808);
		per1.setStreet("2621 Pecksniff Rd");
	}
